...
def project_file_1() -> Path:
    project_file: Path = Path("test_data/week_2_files/Project 1.md")
...
def week_1_notes(temp_dir: Path) -> Generator[WeeklyNotes]:
    ...
    src_filepath: Path = Path("test_data/week_1_files/Test Week 1.md")

    # copy the weekly file to test directory
    dst_filepath: Path = temp_dir / src_filepath.name
    shutil.copy(src_filepath, dst_filepath)

    week_1_notes: WeeklyNotes = load_note_file(file=dst_filepath)
    yield week_1_notes
...
def week_2_notes(temp_dir: Path) -> Generator[WeeklyNotes]:
    # specify the file and the directory to work in
    src_filepath: Path = Path("test_data/week_2_files/Test Week 2.md")

    # copy the weekly file to test directory
    dst_filepath: Path = temp_dir / src_filepath.name
    shutil.copy(src_filepath, dst_filepath)

    week_2_notes: WeeklyNotes = load_note_file(file=dst_filepath)
    yield week_2_notes
